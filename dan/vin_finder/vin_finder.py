...
# --- 配置 Tesseract-OCR 路径 ---
# 在 macOS 上，如果使用 Homebrew 安装，通常不需要手动设置路径。
...
class VinSearchApp(ctk.CTk):
    def __init__(self):
        ...
        self.vin_entry.insert(0, "378888");
    ...
    def handle_found_vin(self, found_vin):
        """
        [线程安全] 此函数只在主线程中执行。
        处理找到目标后的一切事务：停止搜索、弹窗、重置UI。
        """
        self.is_searching = False  # 确保循环会停止
    ...
    def search_loop(self):
        ...
        while self.is_searching and scroll_attempts < max_scrolls:
            ...
            if target_vin in found_numbers:
                # 任务完成！调度主线程处理所有后续操作，然后本线程直接退出。
                self.safe_ui_update(self.handle_found_vin, target_vin)
                return  # 立即退出 search_loop 函数
